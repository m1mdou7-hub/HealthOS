...
create table if not exists public.healthos_billing_invoices (
...
create table if not exists public.healthos_billing_estimates (
  id uuid primary key default gen_random_uuid(),
  owner_user_id uuid not null default auth.uid() references auth.users(id) on delete cascade,
  ...
  converted_invoice_id uuid references public.healthos_billing_invoices(id) on delete set null,
  created_at timestamptz not null default timezone('utc'::text, now()),
  updated_at timestamptz not null default timezone('utc'::text, now()),
  unique (owner_user_id, estimate_number)
);

create table if not exists public.healthos_billing_claims (
  id uuid primary key default gen_random_uuid(),
  owner_user_id uuid not null default auth.uid() references auth.users(id) on delete cascade,
  invoice_id uuid not null references public.healthos_billing_invoices(id) on delete cascade,
  invoice_number text not null,
  patient_id text not null references public.healthos_patients(id) on delete cascade,
  patient_name text not null,
...
create table if not exists public.healthos_billing_payments (
  id uuid primary key default gen_random_uuid(),
  owner_user_id uuid not null default auth.uid() references auth.users(id) on delete cascade,
  invoice_id uuid not null references public.healthos_billing_invoices(id) on delete cascade,
  invoice_number text not null,
...
create table if not exists public.healthos_billing_timeline_events (
  id uuid primary key default gen_random_uuid(),
  owner_user_id uuid not null default auth.uid() references auth.users(id) on delete cascade,
  invoice_id uuid references public.healthos_billing_invoices(id) on delete cascade,
...
create table if not exists public.healthos_billing_settings (
...
create index if not exists healthos_billing_invoices_owner_patient_idx
  on public.healthos_billing_invoices(owner_user_id, patient_id);
create index if not exists healthos_billing_invoices_owner_due_idx
  on public.healthos_billing_invoices(owner_user_id, due_date);
create index if not exists healthos_billing_estimates_owner_patient_idx
  on public.healthos_billing_estimates(owner_user_id, patient_id);
create index if not exists healthos_billing_claims_owner_invoice_idx
  on public.healthos_billing_claims(owner_user_id, invoice_id);
create index if not exists healthos_billing_payments_owner_invoice_idx
  on public.healthos_billing_payments(owner_user_id, invoice_id);
create index if not exists healthos_billing_timeline_owner_occurred_idx
  on public.healthos_billing_timeline_events(owner_user_id, occurred_at desc);

alter table public.healthos_billing_invoices enable row level security;
alter table public.healthos_billing_estimates enable row level security;
alter table public.healthos_billing_claims enable row level security;
alter table public.healthos_billing_payments enable row level security;
alter table public.healthos_billing_timeline_events enable row level security;
alter table public.healthos_billing_settings enable row level security;

drop policy if exists "Billing invoices are isolated by owner" on public.healthos_billing_invoices;
create policy "Billing invoices are isolated by owner"
  on public.healthos_billing_invoices for all to authenticated
  using (
    owner_user_id = auth.uid()
    and exists (
      select 1 from public.healthos_patients
      where healthos_patients.id = healthos_billing_invoices.patient_id
...
drop policy if exists "Billing estimates are isolated by owner" on public.healthos_billing_estimates;
create policy "Billing estimates are isolated by owner"
  on public.healthos_billing_estimates for all to authenticated
  using (
    owner_user_id = auth.uid()
    and exists (
      select 1 from public.healthos_patients
      where healthos_patients.id = healthos_billing_estimates.patient_id
        and healthos_patients.owner_user_id = auth.uid()
    )
  )
  with check (
...
drop policy if exists "Billing claims are isolated by owner" on public.healthos_billing_claims;
create policy "Billing claims are isolated by owner"
  on public.healthos_billing_claims for all to authenticated
  using (
    owner_user_id = auth.uid()
    and exists (
      select 1 from public.healthos_billing_invoices
      where healthos_billing_invoices.id = healthos_billing_claims.invoice_id
        and healthos_billing_invoices.owner_user_id = auth.uid()
    )
  )
  with check (
    owner_user_id = auth.uid()
...
drop policy if exists "Billing payments are isolated by owner" on public.healthos_billing_payments;
create policy "Billing payments are isolated by owner"
  on public.healthos_billing_payments for select to authenticated
  using (
    owner_user_id = auth.uid()
    and exists (
      select 1 from public.healthos_billing_invoices
      where healthos_billing_invoices.id = healthos_billing_payments.invoice_id
        and healthos_billing_invoices.owner_user_id = auth.uid()
    )
  );

drop policy if exists "Billing events are isolated by owner" on public.healthos_billing_timeline_events;
create policy "Billing events are isolated by owner"
  on public.healthos_billing_timeline_events for select to authenticated
  using (owner_user_id = auth.uid());

drop policy if exists "Billing settings are isolated by owner" on public.healthos_billing_settings;
create policy "Billing settings are isolated by owner"
  on public.healthos_billing_settings for all to authenticated
  using (owner_user_id = auth.uid())
...
revoke insert, delete on public.healthos_billing_invoices from authenticated;
revoke update on public.healthos_billing_invoices from authenticated;
grant select on public.healthos_billing_invoices to authenticated;

revoke insert, update, delete on public.healthos_billing_payments from authenticated;
grant select on public.healthos_billing_payments to authenticated;

revoke insert, update, delete on public.healthos_billing_timeline_events from authenticated;
grant select on public.healthos_billing_timeline_events to authenticated;

revoke insert, update, delete on public.healthos_billing_claims from authenticated;
grant select on public.healthos_billing_claims to authenticated;

revoke insert, update, delete on public.healthos_billing_estimates from authenticated;
grant select on public.healthos_billing_estimates to authenticated;
grant select, insert, update, delete on public.healthos_billing_settings to authenticated;
...
create or replace function public.healthos_create_billing_invoice(
...
returns public.healthos_billing_invoices
language plpgsql
security definer
set search_path = public
as $$
declare
  patient_record public.healthos_patients;
  new_invoice public.healthos_billing_invoices;
...
begin
  ...
  from public.healthos_billing_settings
  where owner_user_id = auth.uid();

  insert into public.healthos_billing_invoices (
  ...
  insert into public.healthos_billing_timeline_events (
...
create or replace function public.healthos_record_billing_payment(
...
begin
  ...
  from public.healthos_billing_invoices
  ...
  insert into public.healthos_billing_payments (
  ...
  update public.healthos_billing_invoices
...
create or replace function public.healthos_convert_billing_estimate(
...
as $$
declare
  estimate_record public.healthos_billing_estimates;
  new_invoice public.healthos_billing_invoices;
begin
  select * into estimate_record
  from public.healthos_billing_estimates
  ...
  from public.healthos_create_billing_invoice(
  ...
  update public.healthos_billing_estimates
  set
...
create or replace function public.healthos_resubmit_billing_claim(
  target_claim_id uuid,
  procedure_code text,
  resubmission_notes text
)
returns public.healthos_billing_claims
language plpgsql
security definer
set search_path = public
as $$
declare
  claim_record public.healthos_billing_claims;
begin
  select * into claim_record
  from public.healthos_billing_claims
  ...
  update public.healthos_billing_claims
  set
    status = 'In Review',
...
drop trigger if exists billing_invoices_set_updated_at on public.healthos_billing_invoices;
create trigger billing_invoices_set_updated_at
before update on public.healthos_billing_invoices
for each row execute function public.healthos_set_updated_at();

drop trigger if exists billing_estimates_set_updated_at on public.healthos_billing_estimates;
create trigger billing_estimates_set_updated_at
before update on public.healthos_billing_estimates
for each row execute function public.healthos_set_updated_at();

drop trigger if exists billing_claims_set_updated_at on public.healthos_billing_claims;
create trigger billing_claims_set_updated_at
before update on public.healthos_billing_claims
for each row execute function public.healthos_set_updated_at();

drop trigger if exists billing_settings_set_updated_at on public.healthos_billing_settings;
create trigger billing_settings_set_updated_at
before update on public.healthos_billing_settings
for each row execute function public.healthos_set_updated_at();

revoke all on function public.healthos_create_billing_invoice(text, text, text, jsonb, date, numeric, text, text)
  from public;
revoke all on function public.healthos_record_billing_payment(uuid, numeric, text)
  from public;
revoke all on function public.healthos_convert_billing_estimate(uuid)
  from public;
revoke all on function public.healthos_resubmit_billing_claim(uuid, text, text)
  from public;

grant execute on function public.healthos_create_billing_invoice(text, text, text, jsonb, date, numeric, text, text)
  to authenticated;
grant execute on function public.healthos_record_billing_payment(uuid, numeric, text)
  to authenticated;
grant execute on function public.healthos_convert_billing_estimate(uuid)
  to authenticated;
grant execute on function public.healthos_resubmit_billing_claim(uuid, text, text)
  to authenticated;
