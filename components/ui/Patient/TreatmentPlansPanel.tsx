    ...
    <div className="space-y-6 text-start">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-zinc-900/10 p-4 rounded-3xl border border-zinc-900 gap-3">
        <div>
          ...
          <p className="text-xs text-zinc-400 mt-0.5">{t('tx_coordinator_desc')}</p>
        </div>
        <button
          onClick={() => {
            setEditingPlan(null);
            ...
            return (
              <div key={plan.id} className="p-5 rounded-3xl border border-zinc-900 bg-zinc-950/20 space-y-4">
                ...
                <div className="flex flex-wrap justify-between items-start gap-4">
                  <div className="text-start flex-1 min-w-[200px]">
                    <div className="flex items-center gap-2 flex-wrap">
                      <span className="text-2xs font-mono text-zinc-500 uppercase">{plan.id} • Created {plan.createdDate}</span>
                      ...
                      <span className="text-2xs font-mono text-zinc-400">Dr. {plan.treatingDoctor}</span>
                    </div>
                  ...
                  <div className="text-end flex items-start gap-4 shrink-0 font-mono text-xs">
                    <div>
                      <span className="text-2xs text-zinc-500 block uppercase">{t('est_total_cost')}</span>
                      <span className="text-white font-bold">${plan.estimatedCost.toLocaleString()}</span>
                    </div>
                    <div>
                      <span className="text-2xs text-zinc-500 block uppercase">{t('remaining_balance')}</span>
                ...
                <button
                  ...
                  className="w-full text-center py-1 bg-zinc-900/40 hover:bg-zinc-900 border border-zinc-900/60 rounded-xl text-2xs text-zinc-500 hover:text-zinc-300 transition-colors flex items-center justify-center gap-1 font-mono uppercase"
                >
                  {isExpanded ? (
                ...
                {isExpanded && (
                  <div className="space-y-2 border-t border-zinc-900/40 pt-3">
                    <span className="text-2xs font-mono text-zinc-500 font-bold uppercase block">{t('proc_tooth_map')}</span>
                    <div className="overflow-x-auto rounded-xl border border-zinc-900 bg-zinc-950/40">
                      <table className="w-full text-start text-xs font-mono text-zinc-300">
                        <thead className="bg-zinc-900/20 text-2xs uppercase tracking-wider text-zinc-500 border-b border-zinc-900">
                          <tr>
                          ...
                          {plan.items.map((item, index) => (
                            <tr key={index} className="hover:bg-zinc-900/10">
                              ...
                              <td className="px-4 py-2">
                                <span className={`px-1.5 py-0.2 rounded text-2xs border font-bold uppercase ${
