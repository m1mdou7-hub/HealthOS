      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-zinc-900/10 p-4 rounded-2xl border border-zinc-900 gap-3">
        <div>
          <h3 className="text-sm font-bold text-white flex items-center gap-1.5 font-mono">
            <Clipboard className="w-4 h-4 text-emerald-400" /> Treatment Coordinator
          </h3>
          <p className="text-[11px] text-zinc-400 mt-0.5">Define multi-phase clinical procedures, tooth maps, and payment balances.</p>
        </div>
        <button
          onClick={() => {
            setEditingPlan(null);
        ...
        >
          <Plus className="w-3.5 h-3.5" /> Initialize Plan
        </button>
      </div>
      ...
      <div className="space-y-4">
        {isLoading ? (
          ...
          <div className="text-zinc-500 text-xs text-center py-8 border border-zinc-900 rounded-2xl bg-zinc-950/20">
            No clinical treatment courses logged. Use the toolbar to create a plan.
          </div>
        ) : (
          plans.map((plan) => {
              ...
              <div key={plan.id} className="p-5 rounded-2xl border border-zinc-900 bg-zinc-950/20 space-y-4">
                ...
                <div className="flex flex-wrap justify-between items-start gap-4">
                  ...
                  <div className="text-right flex items-start gap-4 shrink-0 font-mono text-xs">
                    <div>
                      <span className="text-[9px] text-zinc-500 block uppercase">Est. Cost</span>
                ...
                <div className="space-y-1">
                  <div className="flex justify-between text-[10px] font-mono text-zinc-500">
                    <span>Clinical Completion Milestones</span>
                ...
                >
                  {isExpanded ? (
                    <>Hide Detailed Procedures <ChevronUp className="w-3.5 h-3.5" /></>
                  ) : (
                    <>Expand Detailed Procedures <ChevronDown className="w-3.5 h-3.5" /></>
                  )}
                </button>
                ...
                {isExpanded && (
                  <div className="space-y-2 border-t border-zinc-900/40 pt-3">
                    <span className="text-[9px] font-mono text-zinc-500 font-bold uppercase block">Procedure Tooth Map Breakdowns</span>
        ...
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <form onSubmit={handleSavePlan} className="bg-zinc-950 border border-zinc-900 p-6 rounded-2xl w-full max-w-lg space-y-4 text-xs">
            <h3 className="text-sm font-bold text-white border-b border-zinc-900 pb-2">
              {editingPlan ? 'Modify Treatment Plan' : 'Establish Treatment Course'}
            </h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div className="space-y-1 sm:col-span-2">
                <label className="text-zinc-400 font-semibold">Plan Course Title</label>
                <input
                  type="text"
              ...
              <div className="space-y-1 sm:col-span-2">
                <label className="text-zinc-400 font-semibold">Plan Course Description</label>
                <textarea
              ...
              <div className="space-y-1">
                <label className="text-zinc-400 font-semibold">Estimated Cost ($)</label>
                <input
                  type="number"
              ...
              <div className="space-y-1">
                <label className="text-zinc-400 font-semibold">Plan Priority</label>
                <select
              ...
              <div className="sm:col-span-2 space-y-1">
                <label className="text-zinc-400 font-semibold">Procedures (Format: Tooth | Name | Status | Cost | Doctor)</label>
                <textarea
