...
export default function SoapNoteEditor({ supabase, activePatient, demoMode }: SoapNoteEditorProps) {
  ...
  const handleLockNote = (noteId: string) => {
    if (confirm("Are you sure you want to Lock this clinical SOAP note? Locked notes are legally auditable medical records and CANNOT be edited or deleted.")) {
    ...
    try {
      ...
      const res = await fetch("/api/gemini", {
        ...
        body: JSON.stringify({
          action: "soap",
          patientContext,
          prompt: `Create a clean diagnostic crown preparation SOAP note for tooth #11.`
        })
      });
      ...
      <div className="md:col-span-4 space-y-4">
        <div className="p-4 rounded-xl bg-zinc-950 border border-zinc-900 flex flex-col h-[400px]">
          <div className="flex justify-between items-center mb-3">
            <span className="text-[10px] font-mono text-zinc-500 uppercase tracking-widest font-bold">SOAP Records Log</span>
            <button
              onClick={() => {
          ...
          {isLoading ? (
            ...
            <p className="text-zinc-500 text-[11px] py-4">No SOAP notes logged. Click + to begin drafting.</p>
          ) : (
        ...
        {isCreating ? (
          ...
          <form onSubmit={handleCreateNote} className="p-5 rounded-2xl border border-zinc-900 bg-zinc-950/20 space-y-4">
            <div className="flex justify-between items-center border-b border-zinc-900 pb-2">
              <h3 className="text-xs font-bold text-white uppercase font-mono tracking-wider">Draft SOAP Clinical Note</h3>
              <button
                type="button"
              ...
              >
                <Sparkles className="w-3 h-3" /> {aiLoading ? 'Scribing...' : 'AI Scribe Draft'}
              </button>
            </div>
            <div className="space-y-3 text-xs">
              ...
              <div className="space-y-1">
                <label className="text-zinc-400">Subjective (Symptoms / Chief complaint)</label>
                <textarea
              ...
              <div className="space-y-1">
                <label className="text-zinc-400">Objective (Vitals / Diagnostics / Scans)</label>
                <textarea
                  value={soapObjective}
              ...
              <div className="space-y-1">
                <label className="text-zinc-400">Assessment (Diagnosis / Healing stage)</label>
                <textarea
              ...
              <div className="space-y-1">
                <label className="text-zinc-400">Plan (Future treatment courses / home care)</label>
                <textarea
                  value={soapPlan}
          ...
          <div className="p-5 rounded-2xl border border-zinc-900 bg-zinc-950/20 space-y-4">
            <div className="flex justify-between items-start border-b border-zinc-900/60 pb-3">
              <div>
                <span className="text-[9px] font-mono text-zinc-500">Record ID: {activeNote.id} • Dr. {activeNote.author}</span>
              ...
              {!activeNote.locked && (
                ...
                >
                  <Lock className="w-3.5 h-3.5" /> Lock Note
                </button>
              )}
            </div>
