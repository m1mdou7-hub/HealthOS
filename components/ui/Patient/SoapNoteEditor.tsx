      ...
      <div className="md:col-span-4 space-y-4">
        <div className="p-4 rounded-xl bg-zinc-950 border border-zinc-900 flex flex-col h-[400px]">
          <div className="flex justify-between items-center mb-3">
            <span className="text-2xs font-mono text-zinc-500 uppercase tracking-widest font-bold">{t('soap_records_log')}</span>
            <button
              onClick={() => {
          ...
          {isLoading ? (
            ...
            <p className="text-zinc-500 text-xs py-4">{t('no_soap_records')}</p>
          ) : (
              ...
              {notes.map(n => (
                ...
                >
                  <div className="truncate pe-2">
                    <span className="text-2xs font-mono text-zinc-500 block">{n.timestamp}</span>
        ...
        {isCreating ? (
          ...
          <form onSubmit={handleCreateNote} className="p-5 rounded-3xl border border-zinc-900 bg-zinc-950/20 space-y-4">
            <div className="flex justify-between items-center border-b border-zinc-900 pb-2">
              ...
              <button
                ...
                disabled={aiLoading}
                className="px-2.5 py-1 bg-purple-600 hover:bg-purple-500 text-white rounded-lg text-2xs font-bold flex items-center gap-1.5 transition-all shadow-soft shadow-purple-600/10"
              >
              ...
              {activeTooth && (
                <div className="p-3 bg-emerald-500/10 border border-emerald-500/20 rounded-xl flex justify-between items-center text-xs">
                  <span className="text-zinc-300">
                    ...
                    <strong className="text-emerald-400 ms-1">Tooth #{activeTooth} ({activeToothStatus})</strong>
                  </span>
                  <button
                    type="button"
                    onClick={() => {
                    ...
                    className="px-2 py-0.5 bg-zinc-900 border border-zinc-800 text-2xs text-emerald-400 rounded hover:bg-zinc-800 transition-colors"
                  >
          ...
          <div className="p-5 rounded-3xl border border-zinc-900 bg-zinc-950/20 space-y-4">
            <div className="flex justify-between items-start border-b border-zinc-900/60 pb-3">
              <div>
                <span className="text-2xs font-mono text-zinc-500">{t('record_id')}: {activeNote.id} • Dr. {activeNote.author}</span>
                <h3 className="text-sm font-bold text-white mt-0.5">{activeNote.title}</h3>
                <p className="text-2xs text-zinc-400 mt-0.5">{activeNote.timestamp}</p>
              </div>
              {!activeNote.locked && (
                <button
                  onClick={() => handleLockNote(activeNote.id)}
                  className="px-3 py-1 bg-zinc-900 hover:bg-zinc-800 border border-zinc-800 rounded-lg text-rose-400 hover:text-rose-300 text-2xs font-bold flex items-center gap-1 transition-colors"
                >
            ...
            <div className="space-y-4 text-xs font-sans text-start">
              <div>
                <span className="text-2xs font-mono font-bold text-emerald-400 uppercase tracking-wider block mb-1">Subjective</span>
              ...
              <div>
                <span className="text-2xs font-mono font-bold text-emerald-400 uppercase tracking-wider block mb-1">Objective</span>
                <div className="p-3 bg-zinc-950 rounded-xl border border-zinc-900 text-zinc-300 leading-relaxed font-sans">
                  {activeNote.soap?.objective || t('placeholder_objective')}
                </div>
              </div>
              <div>
                <span className="text-2xs font-mono font-bold text-emerald-400 uppercase tracking-wider block mb-1">Assessment</span>
                <div className="p-3 bg-zinc-950 rounded-xl border border-zinc-900 text-zinc-300 leading-relaxed font-sans">
                  {activeNote.soap?.assessment || t('placeholder_assessment')}
                </div>
              </div>
              <div>
                <span className="text-2xs font-mono font-bold text-emerald-400 uppercase tracking-wider block mb-1">Plan</span>
                <div className="p-3 bg-zinc-950 rounded-xl border border-zinc-900 text-zinc-300 leading-relaxed font-sans">
