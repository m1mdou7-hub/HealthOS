    ...
    <div id="operations-search" className="p-6 card-elevated rounded-2xl space-y-6 text-start">
      <div>
        <h3 className="text-sm font-bold font-mono flex items-center gap-2" style={{ color: 'var(--velvet-text)' }}>
          <Search className="w-4 h-4" style={{ color: 'var(--velvet-accent)' }} /> Global Clinical Index Search Engine
        </h3>
      ...
      <div className="relative">
        <Search className="absolute start-4 top-1/2 -translate-y-1/2 w-5 h-5" style={{ color: 'var(--velvet-text-muted)' }} />
        <input 
          type="text"
      ...
      {searchResults ? (
        <div className="space-y-6">
          <p className="text-2xs font-mono font-bold uppercase tracking-widest border-b pb-2" style={{ color: 'var(--velvet-text-muted)', borderColor: 'var(--velvet-border)' }}>
            ...
            <div className="space-y-6">
              ...
              {searchResults.patients.length > 0 && (
                <div className="space-y-2.5">
                  <h4 className="text-xs font-bold font-mono flex items-center gap-1.5 uppercase tracking-wider" style={{ color: 'var(--velvet-text-sub)' }}>
                    <User className="w-3.5 h-3.5" /> Matched Patient Records ({searchResults.patients.length})
                  </h4>
                  <div className="space-y-2">
                    {searchResults.patients.map(p => (
                      <div key={p.id} className="p-3 card-hover rounded-xl border flex justify-between items-center" style={{ background: 'var(--velvet-surface-2)', borderColor: 'var(--velvet-border)' }}>
                        <div>
                          <p className="font-bold text-xs" style={{ color: 'var(--velvet-text)' }}>{p.name}</p>
                          <p className="text-2xs" style={{ color: 'var(--velvet-text-muted)' }}>ID: {p.id} â€¢ Phone: {p.phone}</p>
                        </div>
                        <span className="text-2xs font-mono px-2 py-1 rounded border" style={{ color: 'var(--velvet-text-sub)', background: 'var(--velvet-surface-3)', borderColor: 'var(--velvet-border)' }}>
                          {p.priorityType}
                        </span>
                      </div>
                    ))}
                ...
                <div className="space-y-2.5">
                  ...
                  <div className="space-y-2">
                    {searchResults.appointments.map(a => (
                      <div key={a.id} className="p-3 card-hover rounded-xl border space-y-1" style={{ background: 'var(--velvet-surface-2)', borderColor: 'var(--velvet-border)' }}>
                        <div className="flex justify-between text-xs">
                          <span className="font-bold" style={{ color: 'var(--velvet-text)' }}>{a.patientName}</span>
                          <span className="font-mono" style={{ color: 'var(--velvet-text-muted)' }}>{a.startTime} â€¢ {a.date}</span>
                        </div>
                        <p className="text-2xs font-mono font-bold" style={{ color: 'var(--velvet-accent)' }}>{a.procedure} ({a.chair})</p>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="space-y-2.5">
                  ...
                  <div className="space-y-2">
                    {searchResults.treatments.map(t => (
                      <div key={t.id} className="p-3 card-hover rounded-xl border space-y-1" style={{ background: 'var(--velvet-surface-2)', borderColor: 'var(--velvet-border)' }}>
                        <p className="font-bold text-xs" style={{ color: 'var(--velvet-text)' }}>{t.patientName}</p>
                        <p className="text-2xs leading-relaxed font-mono italic" style={{ color: 'var(--velvet-text-sub)' }}>{t.clinicalNotes}</p>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="space-y-2.5">
                  ...
                  <div className="space-y-2">
                    {searchResults.documents.map((d, i) => (
                      <div key={i} className="p-3 card-hover rounded-xl border flex justify-between items-center" style={{ background: 'var(--velvet-surface-2)', borderColor: 'var(--velvet-border)' }}>
                        <div>
                          <p className="font-semibold text-xs" style={{ color: 'var(--velvet-text)' }}>{d.title}</p>
                          <p className="text-2xs" style={{ color: 'var(--velvet-text-muted)' }}>Patient: {d.patient} â€¢ {d.date}</p>
                        </div>
                        <span className="text-2xs font-mono px-2 py-0.5 rounded-full" style={{ color: 'var(--velvet-accent)', background: 'color-mix(in srgb, var(--velvet-accent) 10%, transparent)', border: '1px solid color-mix(in srgb, var(--velvet-accent) 25%, transparent)' }}>
                          {d.type}
                        </span>
                      </div>
                    ))}
                ...
                <div className="space-y-2.5">
                  ...
                  <div className="space-y-2">
                    {searchResults.aiNotes.map((an, i) => (
                      <div key={i} className="p-3 card-hover rounded-xl border flex justify-between items-center" style={{ background: 'var(--velvet-surface-2)', borderColor: 'var(--velvet-border)' }}>
                        <div>
                          <p className="font-semibold text-xs" style={{ color: 'var(--velvet-text)' }}>{an.title}</p>
                          <p className="text-2xs" style={{ color: 'var(--velvet-text-muted)' }}>Patient: {an.patient}</p>
                        </div>
                        <span className="text-2xs font-mono px-2 py-0.5 rounded-full" style={{ color: 'var(--velvet-success)', background: 'color-mix(in srgb, var(--velvet-success) 10%, transparent)', border: '1px solid color-mix(in srgb, var(--velvet-success) 25%, transparent)' }}>
                          {an.score}
                        </span>
                      </div>
                    ))}
          ...
          {searchResults.patients.length === 0 && 
           ...
           searchResults.documents.length === 0 &&
           searchResults.aiNotes.length === 0 && (
            <div className="p-12 text-center italic text-xs border border-dashed rounded-2xl" style={{ color: 'var(--velvet-text-muted)', borderColor: 'var(--velvet-border)' }}>
              No matching records discovered in the clinical database. Try searching for "Arthur", "implant", or "suture".
            </div>
          )}
        </div>
      ) : (
        <div className="py-12 text-center italic text-xs border border-dashed rounded-2xl" style={{ color: 'var(--velvet-text-muted)', borderColor: 'var(--velvet-border)', background: 'var(--velvet-surface-2)' }}>
